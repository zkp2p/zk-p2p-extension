...
checkBrowsers(paths.appPath, isInteractive)
  ...
  .then(port => {
    ...
    const sendMessage = (type, messages) => {
      if (!devServer.webSocketServer) {
        return;
      }

      devServer.webSocketServer.clients.forEach(client => {
        if (client.readyState === client.OPEN) {
          devServer.sendMessage(client, type, messages);
        }
      });
    };

    // Update the compiler hooks to use the new sendMessage function
    compiler.hooks.done.tap('done', stats => {
      if (stats.hasWarnings()) {
        const warnings = stats.toJson().warnings.map(warning => warning.message || warning);
        sendMessage('warnings', warnings);
      }

      if (stats.hasErrors()) {
        const errors = stats.toJson().errors.map(error => error.message || error);
